...
class Book {
  ...
  static async create(bookData) {
    ...
    let attempts = 0;
    while (attempts < 5) {
      const existing = await this.findByQRCode(qr_code);
      if (!existing) break;
    ...
    if (genre_ids && genre_ids.length > 0) {
      await this.updateGenres(bookId, genre_ids);
    }

    // Link subjects
    if (subject_ids && subject_ids.length > 0) {
      await this.updateSubjects(bookId, subject_ids);
    }
  ...
  // Update book genres
  static async updateGenres(bookId, genreIds) {
  ...
  static async updateSubjects(bookId, subjectIds) {
